...
namespace J6.Controllers
{
    ...
    public class SubCategoriesAPIController : ControllerBase
    {
        ...
        public async Task<ActionResult<IEnumerable<Product>>> getProductsOFSubcategory(int id)
        {
            ...
            var productss = await _context.Products.Where(q => q.SubcategoryId == id).Include(a => a.Promotion).Include(c => c.ProdCarts).Include(p => p.ProdOrders).Include(o => o.ProductImages).Include(i => i.Reviews).Include(e => e.StoreProducts).Include(y => y.Views).ToListAsync();
