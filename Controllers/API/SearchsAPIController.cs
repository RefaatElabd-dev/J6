...
namespace J6.Controllers
{
    ...
    public class SearchsAPiController : ControllerBase
    {
        ...
        public async Task<ActionResult> searchbyname(string name)
        {
            ...
            var products = await _context.Products.Where(e => e.ProductName.Contains(name)).Include(a => a.ProductImages).Include(a => a.Promotion).Include(c => c.Reviews).Include(w => w.ShippingDetail).Include(q => q.ProdCarts).Include(p => p.ProdOrders).ToListAsync();
